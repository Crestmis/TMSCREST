...
function doPost(e){try{var p=e&&e.parameter||{};var action=String(p.action||'');if(action==='insert')return json_(insertTask_(p));if(action==='complete')return json_(completeTask_(p));if(action==='saveAccess')return json_(saveAccess_(p));if(action==='createUser')return json_(createUser_(p));if(action==='updateUser')return json_(updateUser_(p));if(action==='deleteUser')return json_(deleteUser_(p));if(action==='updateTask')return json_(updateTask_(p));if(action==='deleteTask')return json_(deleteTask_(p));if(action==='saveHoliday')return json_(saveHoliday_(p));if(action==='deleteHoliday')return json_(deleteHoliday_(p));if(action==='support')return json_(support_(p));throw new Error('Unsupported POST action: '+action);}catch(err){return json_({success:false,error:String(err.message||err)});}}

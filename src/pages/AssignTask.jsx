...
export default function AssignTask({session,onChanged,setPage}){
 const canEdit=session?.isAdmin||['Editor','Full Access'].includes(session?.access?.['Assign Task'])
 const [form,setForm]=useState({title:'',type:'delegation',assignee:'',department:session.department==='all'?'':session.department,date:'',time:'09:00',frequency:'One-Time',reminders:false,attachment:false,remarks:''})
 ...
 const submit=async e=>{
   ...
   try{
    ...
    setMessage(safeDate!==form.date?`Sunday is a holiday. Planned date moved to ${safeDate}. Task saved successfully.`:'Task saved successfully.')
 ...
 <form className="panel form-panel" onSubmit={submit}>
  <div className="form-grid">
   ...
   <label>Assign To<input value={form.assignee} onChange={e=>change('assignee',e.target.value)} placeholder="Username / name"/></label>
   <label>Department<input value={form.department} onChange={e=>change('department',e.target.value)} placeholder="Department"/></label>

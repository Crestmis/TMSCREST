...
export default function TaskRow({task,onComplete,onSelect,selected=false,selectable=true}){
 const displayStatus=task.liveStatus||task.status||'Pending';const statusClass=String(displayStatus).toLowerCase().replace(/\s+/g,'-');const done=statusClass==='done'||statusClass==='delay'
 return <div className={`task-row ${selected?'selected':''}`}>
   ...
   <div className="task-main"><b>{task.title||'Untitled task'}</b><div className="task-meta"><span className={`type-pill ${task.type.toLowerCase()}`}>{task.type}</span><span>{task.frequency}</span><span>{task.planned||'No date'}{task.plannedTime?` · ${task.plannedTime}`:''}</span>{task.assignee&&<span>{task.assignee}</span>}{task.requireAttachment&&<span><Paperclip size={11}/> Attachment required</span>}</div></div>
